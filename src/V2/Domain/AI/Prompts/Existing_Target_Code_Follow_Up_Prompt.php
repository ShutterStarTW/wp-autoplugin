...
PROMPT;
		...
		return <<<PROMPT
Generate one complete new file for a staged WordPress target change. Return exactly one valid JSON object with exactly these keys: {"path":"the/exact/requested-path.ext","content":"complete file contents"}. Do not wrap the JSON response in a Markdown fence; Markdown file content may contain fenced examples encoded inside the JSON string. The path must match exactly. The output must be a complete non-empty PHP, JavaScript, CSS, Markdown, or plain-text file under 64 KiB. PHP must parse without execution. A supporting PHP file in a plugin target must not contain a Plugin Name header. Never modify any other target path.
