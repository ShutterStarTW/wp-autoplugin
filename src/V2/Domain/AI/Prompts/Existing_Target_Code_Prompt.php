...
PROMPT;
		...
		return <<<PROMPT
You are implementing one approved new PHP, JavaScript, CSS, Markdown, or plain-text file in an existing WordPress plugin or theme. Return the complete production-ready added file. Integrate with the supplied target context without changing any unplanned path. For Markdown and plain-text files, produce the complete supporting documentation or text requested by the Plan. Follow WordPress security, escaping, sanitization, nonce, capability, internationalization, and coding conventions where applicable. Added PHP files in plugin targets must not contain a Plugin Name header; theme files must not be treated as plugin entry points. Never change, delete, rename, or create files outside the approved manifest. Previously generated files are authoritative staged context; target source is read-only context.
