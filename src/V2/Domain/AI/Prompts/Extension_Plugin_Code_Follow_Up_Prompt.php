...
{"outcome":"changes","content":"A concise Markdown change summary.","resolved_request":"One concrete statement of the newest requested result, with contextual references resolved.","acceptance_criteria":["Observable requirement the generated revision must satisfy."],"manifest":{"plugin_name":"...","main_file":"root-file.php","files":[{"path":"relative/path.php","description":"Purpose."}]},"changes":[{"path":"relative/path.php","operation":"update","instruction":"Specific bounded implementation instruction that directly satisfies the resolved request."}]}

For a change, provide one resolved_request and one to eight concrete, testable acceptance_criteria derived from the newest message and its clear conversational antecedent. The manifest describes the desired retained and new extension files after the change. Omitting an existing extension path never deletes it; the server preserves omitted parent files. Deleting an existing extension file requires a changes row with operation:"delete" and no instruction, and the path should be omitted from the manifest. A renamed or moved extension file requires one explicit delete row plus one new manifest file and an operation:"add" instruction. Every new file must have one operation:"add" change instruction. Every regenerated existing file must have one operation:"update" change instruction. Include retained files in changes only when their complete contents must be regenerated; all other retained files are copied unchanged. Never delete, rename, or replace a file unless the administrator's newest request requires that topology change. Use only safe relative paths ending in .php, .js, .css, .json, .html, .svg, .xml, .md, or .txt. The server derives each file type from its path; do not return a type field. Keep 1-20 live files, exactly one root-level .php main_file, and one Plugin Name header in that main file only. The extension must remain a separate plugin: never propose, copy, edit, replace, rename, or delete a file in the inspected target or its read-only parent_theme. Do not propose build artifacts, dependencies, binaries, or filesystem operations. A change request must materially alter extension content or topology.
PROMPT;
	}
