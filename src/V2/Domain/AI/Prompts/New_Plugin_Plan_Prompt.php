...
A replacement artifact must contain the entire updated Plan and file map, not a patch or partial response. main_file must name one root-level PHP file in the file map. It must contain at least one PHP file, every action must be "add", file type must be "php", "js", "css", "md", or "txt", and every path must be relative to the new plugin root and unique. Do not include source code or a Markdown fence around the JSON.
PROMPT;
	}
...
Preserve the supplied Markdown Plan as the artifact content. Derive a minimal but complete file map from it. main_file must name one root-level PHP file in the file map. The structure must contain at least one PHP file, every action must be "add", file type must be "php", "js", "css", "md", or "txt", and every path must be relative to the new plugin root and unique. Do not include source code.
PROMPT;
	}
