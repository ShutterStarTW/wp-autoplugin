...
final class Prompt_Attachment_Repository extends Repository {
	 ...
	 */
	public function attach( int $job_id, int $project_id, int $user_id, array $images, array $reuse_ids = [] ) {
		...
		$job = $this->wpdb->get_row(
			$this->wpdb->prepare( 'SELECT project_id, created_by FROM ' . Installer::table( 'jobs' ) . ' WHERE id = %d', $job_id ),
			ARRAY_A
		);
		if ( ! $job || (int) $job['project_id'] !== $project_id || (int) $job['created_by'] !== $user_id ) {
		...
		foreach ( $reuse_ids as $id ) {
			$record = $this->find( $id );
			if ( ! $record || (int) $record['project_id'] !== $project_id || (int) $record['created_by'] !== $user_id ) {
	...
	public function find( int $id, bool $with_content = true ): ?array {
		$columns = $with_content ? '*' : 'id, project_id, created_by, filename, mime_type, byte_size, width, height, sha256, created_at';
	...
	public function for_job( int $job_id, bool $with_content = false ): array {
		$columns = $with_content ? 'a.*' : 'a.id, a.project_id, a.created_by, a.filename, a.mime_type, a.byte_size, a.width, a.height, a.sha256, a.created_at';
	...
	private function hydrate( array $row ): array {
		foreach ( [ 'id', 'project_id', 'created_by', 'byte_size', 'width', 'height' ] as $field ) {
