...
final class ChatGPT_Model_Service {
	...
	public function __construct( private ?ChatGPT_Token_Manager $tokens = null ) { $this->tokens ??= new ChatGPT_Token_Manager(); }
	...
	public function refresh( ?array $tokens = null ) {
		...
		try {
			...
			$response = wp_safe_remote_get( $url, [ 'timeout' => 30, 'redirection' => 0, 'headers' => array_merge( ChatGPT_Token_Manager::headers( $tokens ), [ 'Accept' => 'application/json' ] ) ] );
			...
			foreach ( $data['models'] as $remote ) {
				...
				$models[ $slug ] = [ 'label' => sanitize_text_field( (string) ( $remote['display_name'] ?? $fallback['label'] ) ), 'levels' => $levels, 'default' => $default ];
			}
			$cache = [ 'account' => $this->account_hash( $tokens ), 'fetched_at' => time(), 'attempted_at' => time(), 'error' => '', 'reconnect_required' => false, 'models' => $models ];
	...
	public function lock(): ChatGPT_Option_Lock { return new ChatGPT_Option_Lock( self::LOCK_OPTION, 45 ); }

	public function delete(): void { delete_option( self::OPTION ); }
	...
	private function record_error( string $message, array $tokens, bool $reconnect = false ): \WP_Error {
		...
		$cache = is_array( $previous ) && hash_equals( (string) ( $previous['account'] ?? '' ), $this->account_hash( $tokens ) ) ? $previous : [ 'account' => $this->account_hash( $tokens ), 'fetched_at' => 0, 'models' => [] ];
		$cache['attempted_at'] = time();
		$cache['error'] = $message;
		$cache['reconnect_required'] = $reconnect;
		update_option( self::OPTION, $cache, false );
		return new \WP_Error( 'chatgpt_models_failed', $message, [ 'status' => 502, 'reconnect_required' => $reconnect ] );
	}
