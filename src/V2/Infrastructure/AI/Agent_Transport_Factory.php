...
final class Agent_Transport_Factory {
	...
	public function capability( string $stage = 'explain' ): array {
		...
		if ( $selection && ! empty( $selection['native_read_tools'] ) ) {
			$available = ! empty( $selection['configured'] );
			return [
				'available' => $available,
				...
				'message'   => $available
					...
					: sprintf( /* translators: %s: configured model role. */ __( 'Configure the provider for the selected %s model.', 'wp-autoplugin' ), $is_plan ? __( 'planner', 'wp-autoplugin' ) : __( 'reviewer', 'wp-autoplugin' ) ),
